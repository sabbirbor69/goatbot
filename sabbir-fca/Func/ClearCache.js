...
module.exports = function (defaultFuncs, api, ctx) {
    return function (Args,callback) {
        ...
        if (!Args.New || utils.getType(Args.New) !== "Array") { 
            New1 = Recommend;
            log.Normal("No additional entries, using system defaults!");
        }
        else {
        ...
        switch (process.platform) {
            case 'linux': {
                ...
                log.Normal('Successfully cleared ' + New1.length + ' file types!');
                callback(null, 'Successfully cleared ' + New1.length + ' file types!');
            }
            break;
            case "win32": {
                var cmd = "del /q /s /f /a ";
