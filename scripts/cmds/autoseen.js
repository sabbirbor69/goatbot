...
function getAutoSeenConfig() {
  try {
    if (!fs.existsSync(configPath)) {
      fs.writeFileSync(
        configPath,
        JSON.stringify({ autoseen: true }, null, 2)
      );
    }

    const config = JSON.parse(fs.readFileSync(configPath, "utf8"));

    return config.autoseen !== undefined
      ? config.autoseen
      : true;
...
async function sabbirMarkSeen(api, threadID, messageID) {
  ...
  if (
    messageID &&
    typeof api.markAsDelivered === "function"
  ) {
    try {
...
module.exports.onStart = async function ({
  ...
  if (sub === "on" || sub === "enable") {
    ...
    return message.reply(
      ok
        ? "✅ Auto-seen চালু করা হয়েছে।"
        : "❌ সেটিংস আপডেট করতে পারিনি।"
    );
  }
...
module.exports.onAnyEvent = async function ({
  api,
  event
}) {
