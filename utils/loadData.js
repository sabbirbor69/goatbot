...
module.exports = async function (api, createLine) {
	...
	log.info(
		...
		getText(
			...
			global.db.allThreadData.filter(
				t => t.threadID.toString().length > 15
			).length
		)
	);
	...
	if (
		api &&
		global.GoatBot.config.database.autoSyncWhenStart == true
	) {

		console.log(
			chalk.hex("#f5ab00")(
				createLine("AUTO SYNC")
			)
		);
		...
		try {
			...
			for (const threadInfo of allThreadInfo) {
				...
				if (
					...
					allThreadData.splice(
						allThreadData.findIndex(
							thread =>
							thread.threadID ===
							threadInfo.threadID
						),
	...
	api.listenMqtt(async (err, event) => {
		...
		try {
			...
			if (
				...
				console.log(
					chalk.green(
						`✓ Auto refreshed thread: ${event.threadID}`
					)
				);
			}
